 ...
 */
import { useRef, useMemo, MouseEventHandler, ReactNode, useImperativeHandle, forwardRef } from 'react';
...
export const Img = observer(
  forwardRef((props: ImgProps, ref) => {
    const { src, alt, borderRadius, style = {}, cursor, isNoTheme, className, isImgLayzed = true, isDefaultBg, ...rest } = props;
    ...
    const url = useMemo(() => {
      ...
      return imgDef;
    }, [isNoTheme, src, theme, isImgLayzed]);
    ...
    const dataUrl = useMemo(() => {
      ...
      return '';
    }, [theme, src, isNoTheme, isImgLayzed]);
    ...
    return (
      <div ref={divRef} className={`${className} ${isDefaultBg ? `img-${theme}-default` : ''}`}>
        <img
...
export const BgImg = observer(
  forwardRef((props: BgImgProps, ref) => {
    ...
    return (
      <div ref={divRef} className={`bg-img-defalut ${className}`} data-img-url={dataBgUrl} style={{ backgroundImage: `url(${dataBgUrl || imgDef})` }} {...rest}>
        {children}
      </div>
    );
  })
