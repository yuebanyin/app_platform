...
/**
 * 
 * @formatNumberWithCommas 数字每三位添加一个，隔开符号 
 * @param number 输入的数字
 * @returns 返回格式化后
 */
export const formatNumberWithCommas = (number: string) => number.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ',');

/**
 * 
 * @formatNumberWithDecimal 规范数字保留小数位的方法
 * @param number 传入的数字
 * @param num  需要保留小数的个数
 * @returns 
 */
export const formatNumberWithDecimal = (number, num) => number.toFixed(num);
